...
libs: $(UNIX_DIR)/$(LIBNAME).so $(i386_DIR)/$(LIBNAME).dll $(WINE_UNIX_ARCH_DIR)/$(LIBNAME).dll

defs: $(i386_DIR)/$(LIBNAME).def $(WINE_UNIX_ARCH_DIR)/$(LIBNAME).def
...
$(UNIX_DIR)/$(LIBNAME).so: $(UNIX_DIR)/unixlib.o $(FTD2XX_DIR)/libftd2xx.a
	$(CC) -o $@ $^ $(LDFLAGS)
...
$(i386_DIR)/lib$(LIBNAME).a: $(LIBNAME).spec
...
$(x86_64_DIR)/lib$(LIBNAME).a: $(LIBNAME).spec
...
$(aarch64_DIR)/lib$(LIBNAME).a: $(LIBNAME).spec
